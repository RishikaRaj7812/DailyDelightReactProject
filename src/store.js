...
// try {
//   localStorageCart = saveCart ? JSON.parse(saveCart) : [];
//   if (!Array.isArray(localStorageCart)) {
//     localStorageCart = []; // Ensure it's an array
//   }
// } catch (error) {
//   console.error("Error parsing cart from localStorage:", error);
//   localStorageCart = []; // Fallback to empty array on error
// }
...
// // // Updated user slice to track logged-in user IDs
// // const userSlice = createSlice({
// //   name: 'user',
// //   initialState: {
// //     userId: null, // Current user ID
// //     userData: null, // Current user data
// //     loggedInUsers: [], // List of logged-in user IDs and their data
// //   },
// //   reducers: {
// //     login(state, action) {
...
// //       // Add user to loggedInUsers if not already present
// //       if (!state.loggedInUsers.some(user => user.userId === userId)) {
// //         state.loggedInUsers.push({ userId, userData });
// //       }
// //     },
// //     logout(state, action) {
// //       const userIdToLogout = action.payload;
// //       // Remove the user from loggedInUsers
// //       state.loggedInUsers = state.loggedInUsers.filter(user => user.userId !== userIdToLogout);
// //       // If the current user is the one logging out, clear current user data
// //       if (state.userId === userIdToLogout) {
// //         state.userId = null;
// //         state.userData = null;
// //       }
// //     },
...
//   localStorage.setItem("cart", JSON.stringify(state.cart)); // Fixed typo: store.Cart -> state.cart
// });
...
// Load loginHistory from localStorage
const loadLoginHistory = () => {
  const savedHistory = localStorage.getItem("loginHistory");
  try {
    return savedHistory ? JSON.parse(savedHistory) : [];
  } catch (error) {
    console.error("Error parsing loginHistory from localStorage:", error);
    return [];
  }
};
...
// Keep the existing cart localStorage subscription
store.subscribe(() => {
  const state = store.getState();
